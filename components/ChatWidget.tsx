...
const ChatWidget: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-end">
      {isOpen && (
        <div className="w-[380px] h-[550px] bg-white rounded-3xl shadow-2xl border border-primary/10 flex flex-col overflow-hidden animate-fade-in-up mb-4">
          <div className="p-6 bg-primary flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-800 font-medium uppercase mt-1 tracking-widest">Always Online</p>
              </div>
            </div>
            ...
            {messages.map((m, idx) => (
              <div key={idx} className={`flex ${m.isBot ? 'justify-start' : 'justify-end'}`}>
                <div className={`max-w-[85%] p-4 rounded-2xl text-sm font-medium ${
                  m.isBot ? 'bg-white text-slate-700 shadow-sm border border-slate-100 rounded-tl-none' : 'bg-slate-900 text-white rounded-tr-none'
                }`}>
                  {m.text}
                </div>
              </div>
          ...
          <form onSubmit={handleSend} className="p-4 bg-white border-t border-slate-100">
            <div className="flex gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-200">
              <input 
                ...
                className="flex-grow bg-transparent border-none focus:ring-0 text-sm p-3 text-slate-700"
              />
              <button className="bg-primary text-slate-900 p-3 rounded-xl shadow-lg hover:scale-105 active:scale-95 transition-all">
      ...
      <button 
        ...
        className="w-16 h-16 bg-primary rounded-2xl shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all shadow-primary/40 group relative"
      >
